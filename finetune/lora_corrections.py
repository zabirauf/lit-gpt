...
def setup(
    data_dir: Path = Path("/notebooks/corrections-slm"),
    checkpoint_dir: Path = Path("checkpoints/stabilityai/stablelm-zephyr-3b"),
    out_dir: Path = Path("/notebooks/corrections-slm/lora/corrections_run_2"),
...
def main(fabric: L.Fabric, data_dir: Path, checkpoint_dir: Path, out_dir: Path) -> None:
    ...
    val_data = torch.load(data_dir / "validation.pt")
...
def validate(fabric: L.Fabric, model: GPT, val_data: List[Dict], tokenizer: Tokenizer, max_iters: int) -> torch.Tensor:
    ...
    # Instead of creating the sample, pick one out of random from val_data
    sample = random.choice(val_data)
    fabric.print(sample["input"])
